...
class Effect extends VantaBase {
  ...
  onInit() {
    if (!THREE) {
      //  a band aid for SSR
      // try to get the THREE object from user options if it's not in the window
      console.log(
        "%cVANTA: %cThree.js not found on window, attempting to load from user options",
        "color: #ff8820; font-weight: bold;",
        "color: white; font-weight: bold;"
      )

      THREE = this.options.THREE || THREE
    }
